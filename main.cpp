...
void cond2(bdd& tree, const bdd p[M][N][N])
{
  for (std::size_t i = 0; i < N; i++)
  {
	...
	tree &= !(p[2][i][7] ^ p[3][i][3]);
	tree &= !(p[0][i][1] ^ p[1][i][5]);
	tree &= !(p[1][i][2] ^ p[2][i][5]);
	//additional constraints
	tree &= !(p[3][i][0] ^ p[1][i][8]);
	tree &= !(p[0][i][1] ^ p[2][i][1]);

	tree &= !(p[0][i][6] ^ p[1][i][4]);

	tree &= !(p[0][i][7] ^ p[2][i][3]);//16
	tree &= !(p[3][i][1] ^ p[1][i][3]);//8
	tree &= !(p[2][i][6] ^ p[1][i][1]);//4
	tree &= !(p[0][i][5] ^ p[1][i][8]);//2
	tree &= !(p[1][i][5] ^ p[3][i][7]);//1
  }
}
...
void cond3(bdd& tree, const bdd p[M][N][N])
{
  ...
  for (std::size_t i = 0; i < N; i++)
  {
	...
	if ((i + 1) % ROW_LENGTH == 0) //right contraint with gluing
	{
	...
	if (i % ROW_LENGTH == 0) //left contraint with gluing
	{
	  index = i + ROW_LENGTH - 1;
	  tree &= !(p[2][i][7] ^ p[3][index][5]);
	}
...
void cond4(bdd& tree, const bdd p[M][N][N])
{
  ...
  for (std::size_t i = 0; i < N; i++)
  {
	if (((i + 1) % ROW_LENGTH != 0) && (i < N - ROW_LENGTH) && (i >= ROW_LENGTH)) // all neighbors
	{
	  ...
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]) | !(p[3][i][4] ^ p[1][top_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]) | !(p[3][i][3] ^ p[0][top_right_index][8]);
	  tree &= !(p[0][i][2] ^ p[0][down_right_index][5]) | !(p[0][i][2] ^ p[0][top_right_index][5]);
	  tree &= !(p[0][i][3] ^ p[0][down_right_index][0]) | !(p[0][i][3] ^ p[0][top_right_index][0]);
	  tree &= !(p[2][i][7] ^ p[3][down_right_index][6]) | !(p[2][i][7] ^ p[3][top_right_index][6]);
	  tree &= !(p[3][i][5] ^ p[0][down_right_index][4]) | !(p[3][i][5] ^ p[0][top_right_index][4]);
	}
	else if (((i + 1) % ROW_LENGTH == 0) && (i < N - ROW_LENGTH) && (i >= ROW_LENGTH)) // all neghbors wigh gluing
	{
	  top_right_index = i - ROW_LENGTH - ROW_LENGTH + 1;
	  down_right_index = i + 1;
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]) | !(p[3][i][4] ^ p[1][top_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]) | !(p[3][i][3] ^ p[0][top_right_index][8]);
	  tree &= !(p[0][i][2] ^ p[0][down_right_index][5]) | !(p[0][i][2] ^ p[0][top_right_index][5]);
	  tree &= !(p[0][i][3] ^ p[0][down_right_index][0]) | !(p[0][i][3] ^ p[0][top_right_index][0]);
	...
	else if (i < ROW_LENGTH && (i + 1) % ROW_LENGTH != 0) // only down right neighbor without gluing
	{
	  down_right_index = i + 1 + ROW_LENGTH;
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]);
	  tree &= !(p[0][i][2] ^ p[0][down_right_index][5]);
	  tree &= !(p[0][i][3] ^ p[0][down_right_index][0]);
	  tree &= !(p[2][i][7] ^ p[3][down_right_index][6]);
	  tree &= !(p[3][i][5] ^ p[0][down_right_index][4]);
	}
	else if (i < ROW_LENGTH && (i + 1) % ROW_LENGTH == 0) // only down right neighbor with gluing
	{
	  down_right_index = i + 1;
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]);
	...
	else if (i >= N - ROW_LENGTH && (i + 1) % ROW_LENGTH != 0) // only top right neighbor without gluing
	{
	  down_right_index = i + 1 - ROW_LENGTH;
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]);
	...
	else if (i >= N - ROW_LENGTH && (i + 1) % ROW_LENGTH == 0) // only top right neighbor with gluing
	{
	  down_right_index = i + 1 - ROW_LENGTH - ROW_LENGTH;
	  tree &= !(p[3][i][4] ^ p[1][down_right_index][5]);
	  tree &= !(p[3][i][3] ^ p[0][down_right_index][8]);
	  tree &= !(p[0][i][2] ^ p[0][down_right_index][5]);
	  tree &= !(p[0][i][3] ^ p[0][down_right_index][0]);
